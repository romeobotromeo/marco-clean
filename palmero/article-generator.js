/**
 * Palmero Daily Article Generator
 * Generates a 600-900 word neighborhood article using Claude,
 * rotating through 30 keywords. Stores result in palmero_articles.
 */
...
async function generateArticle(pool, keyword) {
  ...
  const prompt = `You are writing a neighborhood guide article for a property listing site at 4175 Palmero Dr, Los Angeles, CA 90065 in Mount Washington / Highland Park.
...
- Provides real, useful information about the Highland Park / Mount Washington / NELA area
...
The body_html should use only <p>, <h2>, <ul>, <li> tags. No other tags.`;
  ...
  // Insert into DB (skip if keyword already ran today)
  await pool.query(`
    INSERT INTO palmero_articles (slug, keyword, title, meta_desc, body_html, word_count, published_at)
    VALUES ($1, $2, $3, $4, $5, $6, NOW())
    ON CONFLICT (slug) DO NOTHING
  `, [slug, keyword, parsed.title, parsed.meta_desc || '', parsed.body_html, wordCount]);

  console.log(`[PALMERO] Article generated: "${parsed.title}" (${wordCount} words, slug: ${slug})`);
  return { slug, keyword, title: parsed.title, wordCount };
}

async function runDailyArticle(pool) {
  const keyword = getKeywordForDate();
  console.log(`[PALMERO] Daily article — keyword: "${keyword}"`);
  try {
    return await generateArticle(pool, keyword);
  } catch (err) {
    console.error('[PALMERO] Article generation failed:', err.message);
    return null;
  }
}
